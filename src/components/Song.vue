...
export default {
  ...
  data () {
		return {
			...
			comments: [],
			sort: '1', // indicate that the order should be - by default - latest to oldest
		};
  },
  computed: {
  ...
  methods: {
  	...
  	async addComment(values, { resetForm }) {
  		this.comment_in_submission = true;
			...
			this.comment_alert_color = 'bg-blue-500';
			...
			this.getComments(); // update comment list with newly added conment

			this.comment_in_submission = false;
			this.comment_alert_color = 'bg-green-500'
  ...
  watch: { // tracks changes of properties in this component
  	sort(newVal) {
  		...
  		// use the push function to add query parameters to the current URL
  		this.$router.push({
  			query: {
  				sort: newVal,
  			},
